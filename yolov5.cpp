...
int main(int argc, char** argv) {
	...
	//argv[2] = "E:/work_project_code/yolov5_serialize/weights/best.wts";
	//argv[3] = "E:/work_project_code/yolov5_serialize/weights/yolov5s_67.engine";
	//argv[4] = "s";     // serialize the small model

	argc = 4;
	argv[1] = "-d";    // mean inference model     
	argv[2] = "E:/work_project_code/yolov5_serialize/models/yolov5s.engine";
	argv[3] = "E:/work_project_code/yolov5_serialize/test_data/images";
	argv[4] = "s";     // 
    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        for (int b = 0; b < fcount; b++) {
			auto start = std::chrono::system_clock::now();
			std::string imgpath = img_dir + "/" + file_names[f - fcount + 1 + b];
			std::string imgPath = "E:/work_project_code/yolov5_serialize/test_data/images/10_3011.jpg";
            cv::Mat imgsrc = cv::imread(imgPath);
			if (imgsrc.empty())
			{
				std::cout << "Error! Input image cannot be read...\n";
				continue;
			}		
            imgs_buffer[b] = imgsrc;
            size_t  size_image = imgsrc.cols * imgsrc.rows * 3;
            size_t  size_image_dst = INPUT_H * INPUT_W * 3;
            //copy data to pinned memory
            memcpy(img_host, imgsrc.data,size_image);
            //copy data to device memory
			auto start_c = std::chrono::system_clock::now();
            CUDA_CHECK(cudaMemcpyAsync(img_device,img_host,size_image,cudaMemcpyHostToDevice,stream));
			auto end_c = std::chrono::system_clock::now();
			std::cout << "cudaMemcpy time: " << std::chrono::duration_cast<std::chrono::milliseconds>(end_c - start_c).count() << "ms" << std::endl;

            preprocess_kernel_img(img_device, imgsrc.cols, imgsrc.rows, buffer_idx, INPUT_W, INPUT_H, stream);
            buffer_idx += size_image_dst;
			auto end = std::chrono::system_clock::now();
			std::cout << "preprocess_img time: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << "ms" << std::endl;
        }
        // Run inference
